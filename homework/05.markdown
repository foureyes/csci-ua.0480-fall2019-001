...
# Anonymous Course Reviews (Sessions and Storing Data) - __Thursday, October 16th, by 11PM__
...
👀 🍪First, start off by adding middleware that logs out the cookie header on every request:

* this should display the value in the cookie header on the server's console for every request that comes in
