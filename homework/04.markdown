...
* __home__ - <code>/</code>: home page that displays all of the JS snippets submitted on the site. Can be searched by text that is in a snippet and number of lines.
* __add__ - <code>/add</code>: a page that allows a user to submit a new code snippet with a name, code, and number of lines.
...
<!-- TODO -->
![example interaction](resources/interaction1.gif)
...
Now that we have our data being displayed on the home/main page, let's add functionality to search for snippets based on some text, the number of lines, and a tag. The server will only return snippets that match __all__ of the search criteria (essentially `and`). If a search criteria is blank (empty string), do not include it as a feature to search on.
...
* if searching based on count of lines - return only those snippets that match the number of lines
...
<!-- TODO -->
![searching](resources/interaction1.gif)

And your console should look something like this (for three requests in this example):
...
We now have made our site display JS snippets from in-memory store, and we can search for snippets based on some text, the number of lines that the snippets have, and a tag. Nice! 
...
* now that we have a new form making a <code>POST</code> request to a new URL, add a new route handler in __app.js__ to accept <code>POST</code> requests on the URL of <code>/add</code>
	* inside this handler: 
	    ...
	    3. add the new object to the global array holding all of the snippets. 
	    4. __Important__: the new object should be added to the top of the array (so that it shows up first as most recent on the home/main page)
	    2. after modifying the in-memory store, redirect to the home/main page to display the updated list

__Example interaction:__

<!-- TODO -->
![adding](resources/interaction2.gif)
